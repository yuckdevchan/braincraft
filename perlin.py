...
def generate_world_json():
    ...
    if superflat:
        world = generate_superflat_world(superflat_composition, 8, 8)
    else:
        seed = random.randint(0, 100000000000000000000000000000)
        print("Generating world with seed: " + str(seed))
        world = generate_perlin_noise_2d(8, 8, seed)
...
def generate_perlin_noise_2d(width: int, height: int, seed: int) -> list:
    ...
    for block in world:
        ...
        for ore in treasure:
            if block["coords"][1] >= treasure[ore]["yLower"] and block["coords"][1] <= treasure[ore]["yUpper"]:
...
def generate_random(width: int, height: int) -> list:
    ...
    for x in range(width):
        for y in range(height):
            block = random.choice(["grass_block", "dirt_block", "stone", "bedrock", "air", "diamond_ore", "iron_ore", "coal_ore", "gold_ore"])
            if block != "air":
